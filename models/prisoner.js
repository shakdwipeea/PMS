...
var Prisoner = {
    getAll : (cb) => {
        pool.getConnection((err, conn) => {
            ...
            conn.query("SELECT p.id AS id, p.name AS name, p.date_of_joining AS date_of_joining, p.date_of_release AS date_of_release, " +
                "b.name AS block_name" +
                " FROM prisoner AS p LEFT JOIN block AS b ON p.block_id = b.id",
                (err, rows) => {
                    conn.release();
                    cb(err, rows);
                });
